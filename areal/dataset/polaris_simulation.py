"""Polaris simulation dataset helpers."""
...
def _load_polaris_simulation_split(path: str, split: Optional[str]):
    target_split = split or "train"

    if "polaris_simulation" in path:
        path = "Yuxuan13/polaris_with_responses_16k_demo"
    else:
        path = path
...
def sample_max_new_tokens(n, mean, std, min_val=1, max_val=16384):
    return [max(min(int(random.gauss(mean, std)), max_val), min_val) for _ in range(n)]


def get_polaris_simulation_rl_dataset(
    path: str,
    split: str,
    tokenizer,
    max_length: Optional[int] = None,
):
    dataset = _load_polaris_simulation_split(path=path, split=split)
...
def get_polaris_simulation_sft_dataset(
    path: str,
    split: str,
    tokenizer,
    max_length: Optional[int] = None,
