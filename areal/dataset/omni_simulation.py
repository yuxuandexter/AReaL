"""OmniMath simulation dataset helpers."""
...
def _load_omni_simulation_split(path: str, split: Optional[str]):
    target_split = split or "train"

    if "omni_simulation" in path:
        path = "Yuxuan13/omnimath_with_responses_24k_demo"
    else:
        path = path
...
def sample_max_new_tokens(n, mean, std, min_val=1, max_val=24576):
    return [max(min(int(random.gauss(mean, std)), max_val), min_val) for _ in range(n)]


def get_omni_simulation_rl_dataset(
    path: str,
    split: str,
    tokenizer,
    max_length: Optional[int] = None,
):
    dataset = _load_omni_simulation_split(path=path, split=split)
...
def get_omni_simulation_sft_dataset(
    path: str,
    split: str,
    tokenizer,
    max_length: Optional[int] = None,
